...
I=_=>{
  ...
  fetch("/table.tsv").then(d=>d.text()).then(d=>{
    ...
    .filter(x=>["Tacit", "Dfn"].includes(x[2]) && ["Monadic Function", "Dyadic Function"].includes(x[3]))
    Q()})}
Q=_=>{
  ...
  q.innerHTML=`Write a ${c[3].match(/\w+/)[0].toLowerCase()} ${c[5].toLowerCase()} function, <code>${a?a+" ":""}YourFunction ${w}</code>, that for ${a?E(a)+" and ":""}${E(w)}, solves the following task:<p>${c[1]}`
}
