...
function I(){
  fetch("/table.tsv"
      ...
      if("undefined"===typeof(URLSearchParams)){B().className='w';B().removeChild(w)}
      else{var s=new URLSearchParams(window.location.search);B().className=null!=s.get("w")?"w":"";q.value=s.get("q")};
      Q();Z();})
}
function Z(){z.textContent="Showing "+$("tr:not([hidden])").length+" of "+$("tr").length;}
function W(){B().classList.toggle('w');q.focus()}
function X(){q.value="";Q();q.focus()}
function C(){var qv=q.value;history.replaceState({},D().title,window.location.pathname+(qv?"?q="+encodeURIComponent(qv):""));q.focus()};
function Q(){
